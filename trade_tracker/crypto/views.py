...
def crypto_form_page(request):
    return render(request, "crypto/crypto_form.html")

def crypto_process_form(request):
    if request.method == "POST":
        ...
        if df.empty:
            return render(request, "crypto/crypto_results.html", {"error": "No data found for the given inputs."})
        ...
        if price_change > 0:
            trend = "The crypto coin has been performing well, showing an upward trend."
        else:
            trend = "The crypto coin has been declining, indicating a downward trend."

        # Determine volatility (price fluctuation)
        volatility = df_display['Close'].pct_change().std() * 100 

        if volatility > 2:
            volatility_text = "The crypto coin is highly volatile, meaning prices fluctuate significantly."
        else:
            volatility_text = "The crypto coin is relatively stable with minimal fluctuations."

        # Generate analysis paragraph
        stock_analysis = f"""
        The crypto {stock_name} started at ${start_price:.2f} and closed at ${end_price:.2f}, 
        ...
        return render(request, "crypto/crypto_results.html", {
...
def crypto_download_csv(request):
    full_csv_data = request.session.get("full_csv_data", "")

    if not full_csv_data:
        return HttpResponse("No full crypto data available for download.", content_type="text/plain")

    response = HttpResponse(full_csv_data, content_type="text/csv")
    response["Content-Disposition"] = 'attachment; filename="crypto_data.csv"'
    return response

def crypto_download_correlation(request):
